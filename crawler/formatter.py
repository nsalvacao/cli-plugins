...
from .models import CLIMap, Command, Flag, PositionalArg, EnvVar, Meta
...
def serialize_cli_map(cli_map: CLIMap) -> dict[str, Any]:
    ...
    return {
        ...
        "global_flags": [_serialize_flag(f) for f in sorted(cli_map.global_flags, key=lambda f: f.name)],
        "env_vars": [_serialize_envvar(e) for e in sorted(cli_map.env_vars, key=lambda e: e.name)],
        "tree": {name: _serialize_command(cmd)
                 for name, cmd in sorted(cli_map.tree.items())},
...
def _serialize_command(cmd: Command) -> dict[str, Any]:
    ...
    if cmd.positional_args:
        data["positional_args"] = [
            _serialize_positional_arg(a) for a in cmd.positional_args
        ]
    if cmd.flags:
        data["flags"] = [
            _serialize_flag(f) for f in sorted(cmd.flags, key=lambda f: f.name)
        ]
    if cmd.env_vars:
        data["env_vars"] = [
    ...
    if cmd.subcommands:
        data["subcommands"] = {
            name: _serialize_command(sub)
            for name, sub in sorted(cmd.subcommands.items())
        }
