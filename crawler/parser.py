...
from .parsers.envvars import parse_envvar_section, extract_envvars_from_text
...
def _parse_sectioned(text: str, cli_name: str, command_path: str) -> ParseResult:
    ...
    for section in sections:
        try:
            ...
            logger.warning("Parse error in %s section '%s': %s",
                           command_path, section.header, e)
...
def _parse_arguments(content: str) -> list[PositionalArg]:
    ...
    for line in content.splitlines():
        ...
        if m:
            ...
            results.append(PositionalArg(
                name=name, required=required, description=desc,
            ))
    return results
