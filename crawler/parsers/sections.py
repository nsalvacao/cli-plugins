...
_p(r"^\s*(?:GLOBAL\s+FLAGS|GLOBAL\s+OPTIONS|Global\s+[Ff]lags|Global\s+[Oo]ptions)\s*:?\s*$",
        ...
        if stype is not None:
            ...
            if current_lines or current_header:
                ...
                sections.append(Section(
                    type=current_type, header=current_header,
                    content=content, indent_level=current_indent,
                ))
            elif preamble_lines:
                # Save preamble as description
                content = "\n".join(preamble_lines)
                sections.append(Section(
                    type=SectionType.DESCRIPTION, header="",
                    content=content, indent_level=0,
                ))
                preamble_lines = []
