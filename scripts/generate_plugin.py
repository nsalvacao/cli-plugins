                ...
                for sub_name, sub_data in sorted(subs.items()):
                    ...
                    if sub_subs:
                        for ss_name, ss_data in sorted(sub_subs.items()):
                            ss_desc = _clean_description(
                                ss_data.get("description", "")
                            )
                            ss_path = ss_data.get(
                                "path", f"{sub_path} {ss_name}"
                            )
...
def generate_plugin(
    ...
    *,
    ...
    files[plugin_root / ".claude-plugin" / "plugin.json"] = (
        generate_plugin_json(cli_map, stats)
    )
    files[plugin_root / "skills" / f"cli-{cli}" / "SKILL.md"] = (
        generate_skill_md(cli_map, stats)
    )
    ...
    files[plugin_root / "commands" / "scan-cli.md"] = (
        generate_scan_cli_md(cli, crawler_path)
    )
    files[plugin_root / "scripts" / "rescan.sh"] = (
        generate_rescan_sh(cli, crawler_path)
    )
